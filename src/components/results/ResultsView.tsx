...
export default function ResultsView() {
  const { reset, activeImage, capturedImage } = useSessionStore();
  const [activeTab, setActiveTab] = useState<"visual" | "recipe">("visual");

  const handleExportImage = useCallback(() => {
    const imageToExport = activeImage || capturedImage;
    if (!imageToExport) return;

    const link = document.createElement("a");
    link.href = imageToExport;
    link.download = `aesthetic-result-${Date.now()}.png`;
  ...
  return (
    <div className="h-full flex flex-col bg-stone-50 overflow-hidden fade-in">
      {/* Header with tabs */}
      <div className="bg-white border-b border-stone-200 px-8 pt-8 shrink-0 print:border-none">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-8">
        <div className="max-w-4xl mx-auto h-full">
          ...
          <div className="flex justify-center gap-4 mt-12 pb-12 print:hidden">
            <button
              onClick={handleExportImage}
              disabled={!activeImage && !capturedImage}
              className="px-8 py-4 bg-white border border-stone-200 rounded-full text-[10px] font-black uppercase tracking-[0.2em] text-stone-900 hover:bg-stone-50 transition-all flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <i className="fa-solid fa-download" /> Export Image
            </button>
            <button
